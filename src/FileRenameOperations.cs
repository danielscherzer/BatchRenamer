...
public static class FileRenameOperations
{
	...
	public static string FormatWords(this string fileName, IEnumerable<string> definedWords)
	{
		...
		var splitInput = fileName.SplitIntoWords([' ', '(', ')']);
		...
		sb.Append(fileName[..startMeta].Trim());
	...
	{
		var splitInput = fileName.SplitIntoWords([' ', '.', '_', '(', ')']).Reverse();
	...
	private static readonly Regex outsideASCII = new(@"[\u0000-\u0019\u007F-\uFFFF]");

	public static string RemoveUnicode(this string fileName) => outsideASCII.Replace(fileName, "");
